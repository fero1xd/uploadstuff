...
const UPLOAD_STUFF_VERSION = require("../../../package.json").version as string;
...
export const createNextPageApiHandler = <TRouter extends FileRouter>(
  ...
  return async (req: NextApiRequest, res: NextApiResponse) => {
    ...
    const uploadstuffHook = req.headers["uploadstuff-hook"];
    ...
    if (uploadstuffHook && typeof uploadstuffHook !== "string")
      return res.status(400).send("`uploadstuffHook` must not be an array");
    ...
    res.setHeader("x-uploadstuff-version", UPLOAD_STUFF_VERSION);
